...
function resolveGithub(archiveName, fragment, options, callback, errback) {
    ...
    function fetchBrowserPackageJson(ownerPlusRepo, tag) {
        ...
        var pkgUrl = (token ? github.apiFileContentsUrl(ownerPlusRepo, tag, 'package.json') : github.rawUrl(ownerPlusRepo, tag, 'package.json'));
          var dPkg = q.defer();
          ...
          net.getJson(pkgUrl, makeAuthHeaderOptions(pkgUrl, options)).then(function (pkg) {
                if (pkg) {
                      ...
                      // if (isPrivate) {
							// there is no clean method to download private repo individual files
							// so store this value and process it after a full repo download
					  // 		console.log('Private repo, will store volo URL for cleanup after downloading full repo.');
					  // 		privateUrl = (pkg.volo && pkg.volo.url ? pkg.volo.url : null);
					  // 		dPkg.resolve();
					  // } else {
							pkg.volo.url = 'https://github.com/ExactTarget/fuelux-campaigns/blob/master/dist/fuelux-campaigns.zip?raw=true';
							packageInfo = pkg;
...
//					  }
            ...
            console.log('Cannot download package.json for repo, but continuing anyway.');
            dPkg.resolve();
        });
        ...
        if (fragment || (browserInfo && browserInfo.url)) {
            ...
            //it may just be a directory, so check github first.
              if (fragment) {
					...
					// SP
					// handle fragment case later
					// does it make sense for private or not?
url = github.rawUrl(ownerPlusRepo, tag, fragment);
//                url = (isPrivate ? github.apiFileContentsUrl(ownerPlusRepo, tag, fragment) : github.rawUrl(ownerPlusRepo, tag, fragment));
                //Confirm it is for a single file. If get a 200, then
                //it is a real single file (probably .js file). Otherwise
                //the fragment is
                    return net.head(url, makeAuthHeaderOptions(url), options).then(function (response) {
                    var index;
                  ...
                  // GH File Contents API limited to < 1 MB in size
                    if (response.statusCode >= 200 &&
                        response.statusCode < 300 &&
                        parseInt(response.headers['content-length'], 10) <= 1000) {

						 // if private, must still treat as an archive b/c normal download
						  // will fail
						  if (isPrivate) {
								setAsArchive();
						  } else {
                        ...
                        //Not a single js file or too large for API
                        setAsArchive();
                    }
                });
            } else {
